...
struct Ban *find_ban(struct Client *cptr, struct Ban *banlist)
{
  ...
  for (found = NULL; banlist; banlist = banlist->next)
  {
    ...
    if (!((banlist->flags & BAN_IPMASK) && ipmask_check(&cli_ip(cptr), &banlist->address, banlist->addrbits)) && match(hostmask, cli_user(cptr)->host) && match(hostmask, iphost) && !(sr && !match(hostmask, sr)))
      continue;
...
const char *find_no_nickchange_channel(struct Client *cptr)
{
  if (MyUser(cptr))
  {
         ...
         member = member->next_channel)
    {
      if (IsVoicedOrOpped(member))
        continue;
      if ((member->channel->mode.mode & MODE_MODERATED) || (member->channel->mode.mode & MODE_REGONLY && !IsAccount(cptr)) || is_banned(member))
...
modebuf_flush_int(struct ModeBuf *mbuf, int all)
{
  ...
  if ((mbuf->mb_rem & MODE_DELJOINS) && !(mbuf->mb_channel->mode.mode & (MODE_DELJOINS | MODE_WASDELJOINS)) && find_delayed_joins(mbuf->mb_channel))
  {
  ...
  if ((mbuf->mb_add & MODE_DELJOINS) && (mbuf->mb_channel->mode.mode & MODE_WASDELJOINS))
  {
    mbuf->mb_channel->mode.mode &= ~MODE_WASDELJOINS;
    mbuf->mb_add &= ~MODE_WASDELJOINS;
    mbuf->mb_rem |= MODE_WASDELJOINS;
  ...
  for (i = 0; i < mbuf->mb_count; i++)
  {
    ...
    {
      tmp = (mbuf->mb_dest & MODEBUF_DEST_NOKEY ? 1 : strlen(MB_STRING(mbuf, i)));
                       ...
                       MODEBUF_DEST_LOG))
  {
    ...
    for (i = 0; i < mbuf->mb_count; i++)
    {
      ...
      /* deal with keys... */
        ...
        build_string(strptr, strptr_i, mbuf->mb_dest & MODEBUF_DEST_NOKEY ? "*" : MB_STRING(mbuf, i), 0, ' ');
    ...
    if (mbuf->mb_dest & MODEBUF_DEST_HACK2)
      sendto_opmask_butone(0, SNO_HACK2, "HACK(2): %s MODE %s %s%s%s%s%s%s "
                                         "[%Tu]",
                           cli_name(feature_bool(FEAT_HIS_SNOTICES) ? mbuf->mb_source : app_source),
  ...
  if (mbuf->mb_dest & MODEBUF_DEST_SERVER)
  {
    ...
    for (i = 0; i < mbuf->mb_count; i++)
    {
      ...
      if ((MB_TYPE(mbuf, i) & MODE_CHANOP) && MB_OPLEVEL(mbuf, i) < MAXOPLEVEL)
 ...
 */
void modebuf_mode_uint(struct ModeBuf *mbuf, unsigned int mode, unsigned int uint)
{
  assert(0 != mbuf);
  assert(0 != (mode & (MODE_ADD | MODE_DEL)));
 ...
 */
void modebuf_mode_string(struct ModeBuf *mbuf, unsigned int mode, char *string,
                         int free)
{
  assert(0 != mbuf);
  assert(0 != (mode & (MODE_ADD | MODE_DEL)));
...
send_notoper(struct ParseState *state)
{
  ...
  send_reply(state->sptr, (state->flags & MODE_PARSE_NOTOPER) ? ERR_CHANOPRIVSNEEDED : ERR_NOTONCHANNEL, state->chptr->chname);
...
mode_parse_key(struct ParseState *state, int *flag_p)
{
  ...
  { /* warn if not enough args */
    if (MyUser(state->sptr))
      need_more_params(state->sptr, state->dir == MODE_ADD ? "MODE +k" : "MODE -k");
    return;
  }
  ...
  if (!is_clean_key(state, t_str, state->dir == MODE_ADD ? "MODE +k" : "MODE -k"))
    return;
...
mode_parse_upass(struct ParseState *state, int *flag_p)
{
  ...
  { /* warn if not enough args */
    if (MyUser(state->sptr))
      need_more_params(state->sptr, state->dir == MODE_ADD ? "MODE +U" : "MODE -U");
    return;
  }
  ...
  if (state->flags & MODE_PARSE_FORCE && MyUser(state->sptr) && !HasPriv(state->sptr, PRIV_APASS_OPMODE))
  {
  ...
  if (MyUser(state->sptr) && !(state->flags & MODE_PARSE_FORCE || IsChannelManager(state->member)))
  {
    ...
    else
    {
      send_reply(state->sptr, ERR_NOMANAGER, state->chptr->chname,
                 (TStime() - state->chptr->creationtime < 172800) ? "approximately 4-5 minutes" : "approximately 48 hours");
    }
    return;
  }
  ...
  if (!is_clean_key(state, t_str, state->dir == MODE_ADD ? "MODE +U" : "MODE -U"))
    return;
...
mode_parse_apass(struct ParseState *state, int *flag_p)
{
  ...
  { /* warn if not enough args */
    if (MyUser(state->sptr))
      need_more_params(state->sptr, state->dir == MODE_ADD ? "MODE +A" : "MODE -A");
    return;
  }
  ...
  if (MyUser(state->sptr))
  {
    ...
    if (TStime() - state->chptr->creationtime >= 172800 && state->dir == MODE_DEL && !IsAnOper(state->sptr))
    {
  ...
  if (!is_clean_key(state, t_str, state->dir == MODE_ADD ? "MODE +A" : "MODE -A"))
    return;
...
mode_parse_ban(struct ParseState *state, int *flag_p)
{
  ...
  { /* warn if empty */
    if (MyUser(state->sptr))
      need_more_params(state->sptr, state->dir == MODE_ADD ? "MODE +b" : "MODE -b");
    return;
  }
  ...
  newban->next = 0;
  newban->flags = ((state->dir == MODE_ADD) ? BAN_ADD : BAN_DEL) | (*flag_p == MODE_BAN ? 0 : BAN_EXCEPTION);
...
mode_parse_client(struct ParseState *state, int *flag_p)
{
  ...
  if (MyUser(state->sptr))
  {
    ...
    if (colon != NULL)
    {
      *colon++ = '\0';
      ...
      else if (!(state->flags & MODE_PARSE_FORCE) && state->member && (req_oplevel < OpLevel(state->member) || (req_oplevel == OpLevel(state->member) && OpLevel(state->member) < MAXOPLEVEL) || req_oplevel > MAXOPLEVEL))
      {
  ...
  if (state->flags & MODE_PARSE_BOUNCE && state->dir == MODE_DEL && flag_p[0] == MODE_CHANOP && (member = find_member_link(state->chptr, acptr)))
  ...
  for (i = 0; state->cli_change[i].flag; i++)
  {
        ...
        (MODE_DEL | MODE_CHANOP))
    {
      ...
      if (IsChannelService(state->cli_change[i].client))
      {
        if (state->flags & MODE_PARSE_FORCE) /* it was forced */
          sendto_opmask_butone(0, SNO_HACK4, "Deop of +k user on %H by %s",
                               state->chptr,
                               (IsServer(state->sptr) ? cli_name(state->sptr) : cli_name((cli_user(state->sptr))->server)));
      ...
      if (MyUser(state->sptr))
      {
        ...
        if (state->sptr != state->cli_change[i].client && state->member && ((OpLevel(member) < OpLevel(state->member)) || (OpLevel(member) == OpLevel(state->member) && OpLevel(member) < MAXOPLEVEL)))
        {
               ...
               struct Membership *member)
{
  ...
  while (*modestr)
  {
    for (; *modestr; modestr++)
    {
      ...
      if (!flag_p[0])
      { /* didn't find it?  complain and continue */
        if (MyUser(state.sptr))
    ...
    if (state.parc > 0)
    { /* process next argument in string */
      modestr = state.parv[state.args_used++];
      state.parc--;
 ...
 */
void joinbuf_join(struct JoinBuf *jbuf, struct Channel *chan, unsigned int flags)
{
  unsigned int len;
  int is_local;
      ...
      jbuf->jb_type == JOINBUF_TYPE_PARTALL)
  {
    ...
    if (!(flags & (CHFL_ZOMBIE | CHFL_DELAYED)))
      sendcmdto_channel_butserv_butone(jbuf->jb_source, CMD_PART, chan, NULL, 0,
                                       (flags & CHFL_BANNED || !jbuf->jb_comment) ? ":%H" : "%H :%s", chan, jbuf->jb_comment);
...
int joinbuf_flush(struct JoinBuf *jbuf)
{
  ...
  jbuf->jb_strlen = ((jbuf->jb_type == JOINBUF_TYPE_PART ? STARTJOINLEN : STARTCREATELEN) +
