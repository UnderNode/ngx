...
static void try_connections(struct Event *ev)
{
  ...
  for (aconf = GlobalConfList; aconf; aconf = aconf->next)
  {
     ...
     */
    if (!(aconf->status & CONF_SERVER) || aconf->address.port == 0 || !(aconf->flags & CONF_AUTOCONNECT) || ((ajupe = jupe_find(aconf->name)) && JupeIsActive(ajupe)))
      continue;
     ...
     */
    if (hold || done || (ConfLinks(aconf) > ConfMaxLinks(aconf)) || FindServer(aconf->name) || conf_eval_crule(aconf->name, CRULE_MASK))
      continue;
...
static void check_pings(struct Event *ev)
{
  ...
  for (i = 0; i <= HighestFd; i++)
  {
    ...
    if (!IsRegistered(cptr))
    {
      ...
      if ((CurrentTime - cli_firsttime(cptr) >= max_ping) && auth_ping_timeout(cptr))
        continue;
      if (!IsRegistered(cptr))
      {
...
static void parse_command_line(int argc, char **argv)
{
  ...
  while ((opt = getopt(argc, argv, options)) != EOF)
    ...
    case 'f':
      ...
      break;
    case 'h':
      ircd_strncpy(cli_name(&me), optarg, HOSTLEN);
      break;
    case 'v':
      printf("ircd %s\n", version);
